...
Temporarily changes column type to update data safely
"""
...
def fix_database():
    ...
    try:
        ...
        # Temporarily change status column to VARCHAR to allow updates
        print("🔄 Temporarily changing status column to VARCHAR...")
        cursor.execute("ALTER TABLE bookings MODIFY COLUMN status VARCHAR(20) DEFAULT 'Booked'")
        print("✅ Status column changed to VARCHAR")
        
        # Now update the problematic status values
        ...
        print("🔄 Changing status column back to ENUM...")
        cursor.execute("""
            ALTER TABLE bookings 
            MODIFY COLUMN status ENUM('Booked', 'Consumed', 'Cancelled') DEFAULT 'Booked'
        """)
        print("✅ Status column changed back to ENUM")
